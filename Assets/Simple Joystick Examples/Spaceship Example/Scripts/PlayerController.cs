...
namespace UltimateJoystickExample.Spaceship
{
	public class PlayerController : MonoBehaviour
	{
		...
		void Update ()
		{
			...
			movePosition = SimpleJoystick.GetPosition( "Movement" );
			shootPosition = SimpleJoystick.GetPosition( "Shooting" );
			...
			if( SimpleJoystick.GetJoystickState( "Shooting" ) && shootingTimer <= 0 )
			{
		...
		void FixedUpdate ()
		{
			...
			if( canControl == false )
			{
				// Then reset the player's rotation, position, velocity and angular velocity.
			...
			else
			{
				...
				myRigidbody.AddForce( transform.forward * SimpleJoystick.GetDistance( "Movement" ) * 1000.0f * accelerationSpeed * Time.deltaTime );
		...
		void CheckExitScreen ()
		{
			...
			// If the absolute value of the player's X position is greater than the orthographic size of the camera multiplied by the camera's aspect ratio, then reset the player on the other side.
			...
			// If the absolute value of the player's Z position is greater than the orthographic size, then reset the Z position to the other side.
